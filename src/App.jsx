...
export default function App() {
  ...
  const shuffledExercises = useMemo(() => {
    ...
    return seededShuffle(activeChapter.exercises, sessionSeed + (chapterIndex || 0) + level.length + subject.length);
    ...
    <main>
      <section className="max-w-7xl mx-auto px-6 pt-16 pb-20 grid lg:grid-cols-2 gap-12 items-center">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <p className="mt-7 text-lg md:text-xl text-white/70 max-w-xl leading-relaxed">Cours détaillés, exercices plein écran, QCM et progression.</p>
          <div className="mt-8 flex flex-col sm:flex-row gap-4">
            <Button onClick={isLoggedIn ? () => document.getElementById("cours")?.scrollIntoView({ behavior: "smooth" }) : () => setShowLogin(true)} className="rounded-2xl px-7 py-4 bg-gradient-to-r from-indigo-500 to-fuchsia-500 hover:opacity-90 shadow-xl shadow-indigo-500/20">
          ...
          <div className="rounded-[1.5rem] bg-slate-950/80 border border-white/10 p-5">
            <div className="flex items-center justify-between mb-5">
              <div>
                ...
                <h2 className="text-2xl font-bold">{isLoggedIn ? "Espace connecté" : "Connexion requise"}</h2>
              </div>
              <div className="h-12 w-12 rounded-2xl bg-gradient-to-br from-indigo-500 to-pink-500 flex items-center justify-center">🧠</div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="rounded-2xl bg-white/10 p-4"><p className="text-white/50 text-sm">Chapitres terminés</p><p className="text-3xl font-black mt-2">{completed.length}</p></div>
            </div>
      ...
      <section id="cours" className="max-w-7xl mx-auto px-6 py-16">
        {!isLoggedIn ? (
          ...
          <CourseSelector level={level} setLevel={setLevel} subject={subject} setSubject={setSubject} subjects={subjects} chapters={chapters} openChapter={openChapter} />
        )}
      </section>
    </main>
  );
...
function LockedCourses({ setShowLogin }) {
  return (
    <div className="rounded-[2rem] bg-white text-slate-950 p-10 shadow-2xl text-center max-w-3xl mx-auto">
      ...
      <p className="text-slate-600 mt-4">Connecte-toi ou crée un compte pour accéder aux cours détaillés.</p>
      <Button onClick={() => setShowLogin(true)} className="mt-6 rounded-2xl px-7 py-4 bg-slate-950 text-white">Se connecter</Button>
    </div>
  );
}
    ...
    <>
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-8">
        <div>
          ...
          <p className="text-white/60 mt-3 max-w-2xl">Tous les chapitres présents dans tes fichiers data s’affichent ici.</p>
        </div>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-6 gap-4 mb-8">
        {subjects.map((item) => (
          <button key={item.name} onClick={() => setSubject(item.name)} className={`rounded-3xl p-5 text-left border transition ${subject === item.name ? "bg-white text-slate-950 border-white" : "bg-white/10 border-white/10 hover:bg-white/15"}`}>
            <div className={`h-11 w-11 rounded-2xl bg-gradient-to-br ${item.color} flex items-center justify-center text-white mb-4`}>{item.icon}</div>
      ...
      <div className="rounded-[2rem] bg-white text-slate-950 p-8 shadow-2xl">
        ...
        <h3 className="text-3xl font-black mb-2">Chapitres disponibles</h3>
        <p className="text-slate-500 mb-6">Nombre de chapitres affichés : {chapters.length}</p>
        {chapters.length === 0 ? (
          <div className="rounded-3xl bg-slate-100 p-8 text-slate-600">Aucun chapitre installé pour cette matière pour le moment.</div>
        ) : (
          <div className="grid md:grid-cols-2 gap-5">
            {chapters.map((chapter, index) => (
              <button key={`${chapter.title}-${index}`} onClick={() => openChapter(index)} className="rounded-3xl bg-slate-100 p-6 text-left hover:bg-slate-200 transition border border-slate-200">
                <p className="text-xl font-black mb-3">{chapter.title}</p>
                <p className="text-slate-600 mb-4">{chapter.lesson}</p>
                <ul className="text-sm text-slate-600 space-y-1">{(chapter.objectives || []).map((item) => <li key={item}>✅ {item}</li>)}</ul>
...
function CoursePage({ level, subject, selectedSubject, chapter, setPage, startExercise, completeChapter }) {
  const hasVideo = Boolean(chapter.video && chapter.video.trim() !== "");
    ...
    <main className="max-w-6xl mx-auto px-6 py-12">
      ...
      <div className="rounded-[2rem] bg-white text-slate-950 p-8 md:p-12 shadow-2xl">
        <div className="flex items-center gap-4 mb-8">
          <div className={`h-14 w-14 rounded-2xl bg-gradient-to-br ${selectedSubject.color} flex items-center justify-center text-white text-2xl`}>{selectedSubject.icon}</div>
          <div><p className="text-slate-500">{level} · {subject}</p><h1 className="text-4xl font-black">{chapter.title}</h1></div>
        </div>
        ...
        {hasVideo && (
          <div className="mt-8 rounded-3xl overflow-hidden bg-slate-950 shadow-2xl">
            ...
            <iframe className="w-full aspect-video" src={chapter.video} title={`Vidéo ${chapter.title}`} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen />
          </div>
        )}

        <div className="mt-8 grid md:grid-cols-2 gap-6">
          <div className="rounded-3xl bg-slate-100 p-6"><h2 className="text-2xl font-black mb-4">Objectifs</h2><ul className="space-y-2 text-slate-700">{(chapter.objectives || []).map((o) => <li key={o}>✅ {o}</li>)}</ul></div>
          <div className="rounded-3xl bg-slate-100 p-6"><h2 className="text-2xl font-black mb-4">Méthode</h2><ol className="space-y-2 text-slate-700">{(chapter.method || []).map((m, i) => <li key={m}>{i + 1}. {m}</li>)}</ol></div>
        </div>
        ...
        <div className="mt-6 grid md:grid-cols-2 gap-6">
          <div className="rounded-3xl bg-rose-50 border border-rose-100 p-6"><h2 className="text-2xl font-black mb-4">Erreurs fréquentes</h2><ul className="space-y-2 text-slate-700">{(chapter.details.mistakes || []).map((m) => <li key={m}>⚠️ {m}</li>)}</ul></div>
          <div className="rounded-3xl bg-slate-100 p-6"><h2 className="text-2xl font-black mb-4">Résumé à retenir</h2><p className="text-slate-700 leading-relaxed">{chapter.details.summary}</p><p className="mt-4 text-indigo-700 font-bold">Conseil IA : commence par 2 exercices simples, puis passe au niveau supérieur.</p></div>
        </div>

        <div className="mt-8 flex flex-col sm:flex-row gap-4">
          <Button onClick={startExercise} className="rounded-2xl px-8 py-4 bg-indigo-600 text-white hover:bg-indigo-500">Commencer les exercices plein écran</Button>
          <Button onClick={completeChapter} className="rounded-2xl px-8 py-4 bg-slate-950 text-white hover:bg-slate-800">Marquer le chapitre terminé</Button>
        </div>
      </div>
    </main>
  );
    ...
    <main className="min-h-[80vh] max-w-5xl mx-auto px-6 py-10 flex items-center">
      <div className="w-full rounded-[2.5rem] bg-white text-slate-950 p-8 md:p-14 shadow-2xl">
        <div className="flex justify-between items-start gap-4 mb-8">
          <div><p className="text-indigo-600 font-bold">{level} · {subject}</p><h1 className="text-4xl md:text-5xl font-black mt-2">{chapter.title}</h1><p className="text-slate-500 mt-2">Exercice {(exerciseStep % chapter.exercises.length) + 1} sur {chapter.exercises.length} · {currentExercise.difficulty} · QCM</p></div>
          <Button onClick={() => { setPage("course"); setTimeout(() => window.scrollTo({ top: 0, behavior: "smooth" }), 50); }} className="rounded-2xl bg-slate-100 px-5 py-3">Retour cours</Button>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-4 mb-6">{(currentExercise.choices || []).map((choice) => <button key={choice} onClick={() => { setSelectedChoice(choice); setAnswer(""); }} className={`rounded-3xl border px-6 py-5 text-left text-xl font-bold transition ${selectedChoice === choice ? "bg-indigo-600 text-white border-indigo-600" : "bg-slate-100 text-slate-950 border-slate-200 hover:bg-slate-200"}`}>{choice}</button>)}</div>
        <div className="rounded-3xl bg-slate-50 border border-slate-200 p-4 mb-6"><p className="text-sm text-slate-500 mb-2">Ou écrire une réponse libre</p><input value={answer} onChange={(e) => { setAnswer(e.target.value); setSelectedChoice(""); }} autoFocus placeholder="Écris ta réponse ici..." className="w-full bg-transparent text-2xl outline-none" /></div>
