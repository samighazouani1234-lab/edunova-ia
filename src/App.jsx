...
function makeChapter(title, lesson, objectives, exercises, details = {}) {
  return {
    ...
    exercises: exercises.map(([question, answer], index) => ({ id: `${title}-${index}`, question, answer })),
  };
}
...
export default function App() {
  ...
  function validateExercise() {
    ...
    if (normalize(answer) === normalize(currentExercise.answer)) {
...
function ExercisePage({ level, subject, chapter, currentExercise, exerciseStep, answer, setAnswer, validateExercise, nextExercise, result, showCorrection, setShowCorrection, setPage }) {
  return <main className="min-h-[80vh] max-w-5xl mx-auto px-6 py-10 flex items-center"><div className="w-full rounded-[2.5rem] bg-white text-slate-950 p-8 md:p-14 shadow-2xl"><div className="flex justify-between items-start gap-4 mb-8"><div><p className="text-indigo-600 font-bold">{level} · {subject}</p><h1 className="text-4xl md:text-5xl font-black mt-2">{chapter.title}</h1><p className="text-slate-500 mt-2">Exercice {(exerciseStep % chapter.exercises.length) + 1} sur {chapter.exercises.length} · ordre mélangé à chaque connexion</p></div><Button onClick={() => { setPage("course"); setTimeout(() => window.scrollTo({ top: 0, behavior: "smooth" }), 50); }} className="rounded-2xl bg-slate-100 px-5 py-3">Retour cours</Button></div><div className="rounded-[2rem] bg-slate-950 text-white p-8 md:p-10 mb-8"><p className="text-indigo-300 font-semibold mb-3">Question</p><h2 className="text-3xl md:text-4xl font-black leading-tight">{currentExercise.question}</h2></div><input value={answer} onChange={(e) => setAnswer(e.target.value)} autoFocus placeholder="Écris ta réponse ici..." className="w-full rounded-3xl bg-slate-100 border border-slate-200 px-6 py-5 text-2xl outline-none focus:ring-4 focus:ring-indigo-200" /><div className="mt-6 grid md:grid-cols-3 gap-4"><Button onClick={validateExercise} className="rounded-2xl bg-indigo-600 hover:bg-indigo-500 text-white py-4 text-lg">Valider</Button><Button onClick={nextExercise} className="rounded-2xl bg-slate-950 hover:bg-slate-800 text-white py-4 text-lg">Exercice suivant</Button><Button onClick={() => setShowCorrection(!showCorrection)} className="rounded-2xl bg-slate-100 text-slate-950 py-4 text-lg">Correction</Button></div>{result && <div className="mt-6 rounded-3xl bg-slate-100 p-5 text-lg font-semibold">{result}</div>}{showCorrection && <div className="mt-6 rounded-3xl bg-emerald-50 border border-emerald-100 p-6"><p className="font-black text-xl">Correction</p><p className="text-slate-700 mt-2 text-2xl">{currentExercise.answer}</p></div>}</div></main>;
}
