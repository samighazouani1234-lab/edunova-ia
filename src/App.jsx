...
export default function App() {
  ...
  const [subject] = useState("Mathématiques");
  ...
  const chapters = useMemo(() => {
    const source = level === "Primaire" ? primaryMath : collegeMath;
    ...
    <main>
      ...
      <section id="cours" className="max-w-7xl mx-auto px-6 py-16">
        {!isLoggedIn ? (
          ...
          <CourseSelector level={level} setLevel={setLevel} chapters={chapters} openChapter={openChapter} />
        )}
      </section>
    </main>
  );
  ...
  return (
    <div className="rounded-[2rem] bg-white text-slate-950 p-10 shadow-2xl text-center max-w-3xl mx-auto">
      ...
      <p className="text-slate-600 mt-4">
        Connecte-toi ou crée un compte pour accéder aux cours détaillés.
      </p>
      <Button
        onClick={() => setShowLogin(true)}
...
function CourseSelector({ level, setLevel, chapters, openChapter }) {
  return (
    <>
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-8">
        <div>
          <p className="text-indigo-300 font-semibold">Cours détaillés</p>
          <h2 className="text-4xl font-black mt-2">Mathématiques - {level}</h2>
          <p className="text-white/60 mt-3 max-w-2xl">
            Chaque chapitre ouvre une page complète avec vidéo, méthode, QCM et exercices.
          </p>
        </div>
      ...
      <div className="rounded-[2rem] bg-white text-slate-950 p-8 shadow-2xl">
        <p className="text-sm text-slate-500">{level} · Mathématiques</p>
    ...
    <main className="max-w-6xl mx-auto px-6 py-12">
      ...
      <div className="rounded-[2rem] bg-white text-slate-950 p-8 md:p-12 shadow-2xl">
        <div className="flex items-center gap-4 mb-8">
          <div
            className={`h-14 w-14 rounded-2xl bg-gradient-to-br ${selectedSubject.color} flex items-center justify-center text-white text-2xl`}
          >
          ...
          <div className="mt-8 rounded-3xl overflow-hidden bg-slate-950 shadow-2xl">
            <div className="p-5 text-white">
              ...
              <h2 className="text-2xl font-black mt-1">
                Regarder l’explication avant les exercices
              </h2>
            </div>
...
function ExercisePage({
  ...
  const choices = currentExercise.choices || [currentExercise.answer];
        ...
        <div className="flex justify-between items-start gap-4 mb-8">
          <div>
            ...
            <p className="text-slate-500 mt-2">
              Exercice {(exerciseStep % chapter.exercises.length) + 1} sur {chapter.exercises.length}
              {" "}· QCM · ordre mélangé à chaque connexion
            </p>
          </div>
      ...
      <div className="rounded-[2.5rem] bg-white/10 border border-white/10 p-8 md:p-12 grid lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
          ...
          <h2 className="text-4xl font-black mt-2">
            Cours accessibles uniquement après connexion
          </h2>
...
function Footer() {
  return (
    <footer className="max-w-7xl mx-auto px-6 py-10 text-white/45 text-sm flex flex-col md:flex-row gap-3 md:items-center md:justify-between">
      <p>© 2026 ÉduNova IA.</p>
      <p>Primaire · Collège · Mathématiques · Exercices plein écran</p>
    </footer>
  );
}
