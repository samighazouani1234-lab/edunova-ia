  ...
  return (
    <header className="max-w-7xl mx-auto px-6 py-6 flex items-center justify-between">
      ...
      <div className="flex gap-3">
        {isLoggedIn && (
          <Button onClick={logout} className="rounded-2xl bg-white/10 text-white px-5 py-3 border border-white/10">
            Déconnexion
          </Button>
        )}
      ...
      <section className="max-w-7xl mx-auto px-6 pt-16 pb-20 grid lg:grid-cols-2 gap-12 items-center">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="mt-8 flex flex-col sm:flex-row gap-4">
            ...
            <Button onClick={() => setShowDemo(true)} className="rounded-2xl px-7 py-4 border border-white/15 bg-white/5 text-white hover:bg-white/10">
              Voir la démo ▶️
            </Button>
          </div>
        </motion.div>
...
function LockedCourses({ setShowLogin }) {
  return (
    <div className="rounded-[2rem] bg-white text-slate-950 p-10 shadow-2xl text-center max-w-3xl mx-auto">
      ...
      <p className="text-slate-600 mt-4">Connecte-toi ou crée un compte pour accéder aux cours détaillés et aux exercices plein écran.</p>
      <Button onClick={() => setShowLogin(true)} className="mt-6 rounded-2xl px-7 py-4 bg-slate-950 text-white">
        Se connecter
      </Button>
    </div>
  );
    ...
    <>
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-8">
        <div>
          ...
          <p className="text-white/60 mt-3 max-w-2xl">Chaque chapitre ouvre une page complète avec vidéo, méthode, QCM et exercices plein écran.</p>
        </div>
        <div className="flex rounded-2xl bg-white/10 p-1 border border-white/10 w-fit">
          {levels.map((item) => (
            <button
              ...
              className={`px-5 py-3 rounded-xl text-sm font-semibold transition ${level === item ? "bg-white text-slate-950" : "text-white/70"}`}
            >
              {item}
            </button>
          ))}
        ...
        {subjects.map((item) => (
          <button
            ...
            className={`rounded-3xl p-5 text-left border transition ${
              subject === item.name ? "bg-white text-slate-950 border-white" : "bg-white/10 border-white/10 hover:bg-white/15"
            }`}
          >
  ...
  return (
    <main className="max-w-6xl mx-auto px-6 py-12">
      ...
      <div className="rounded-[2rem] bg-white text-slate-950 p-8 md:p-12 shadow-2xl">
        ...
        <div className="mt-8 flex flex-col sm:flex-row gap-4">
          <Button onClick={startExercise} className="rounded-2xl px-8 py-4 bg-indigo-600 text-white hover:bg-indigo-500">
            Commencer les exercices plein écran
          </Button>
          <Button onClick={completeChapter} className="rounded-2xl px-8 py-4 bg-slate-950 text-white hover:bg-slate-800">
    ...
    <main className="min-h-[80vh] max-w-5xl mx-auto px-6 py-10 flex items-center">
      <div className="w-full rounded-[2.5rem] bg-white text-slate-950 p-8 md:p-14 shadow-2xl">
        ...
        <div className="rounded-[2rem] bg-slate-950 text-white p-8 md:p-10 mb-8">
          ...
          <h2 className="text-3xl md:text-4xl font-black leading-tight">{currentExercise.question}</h2>
        </div>
          ...
          {(currentExercise.choices || []).map((choice) => (
            <button
              ...
              className={`rounded-3xl border px-6 py-5 text-left text-xl font-bold transition ${
                selectedChoice === choice ? "bg-indigo-600 text-white border-indigo-600" : "bg-slate-100 text-slate-950 border-slate-200 hover:bg-slate-200"
              }`}
            >
              {choice}
            </button>
        ...
        <div className="mt-6 grid md:grid-cols-3 gap-4">
          <Button onClick={validateExercise} className="rounded-2xl bg-indigo-600 hover:bg-indigo-500 text-white py-4 text-lg">
            Valider
          </Button>
          <Button onClick={nextExercise} className="rounded-2xl bg-slate-950 hover:bg-slate-800 text-white py-4 text-lg">
            Exercice suivant
          </Button>
          <Button onClick={() => setShowCorrection(!showCorrection)} className="rounded-2xl bg-slate-100 text-slate-950 py-4 text-lg">
            Correction
          </Button>
        </div>
  ...
  return (
    <section className="max-w-7xl mx-auto px-6 py-16 grid lg:grid-cols-2 gap-8 items-center">
      <div>
        ...
        <p className="text-white/70 text-lg leading-relaxed">Le score évolue avec les réponses et permet d’adapter le niveau.</p>
      </div>
      <div className="rounded-[2rem] bg-white text-slate-950 border-0 shadow-2xl p-8">
        ...
        <input type="range" min="0" max="100" value={score} onChange={(e) => setScore(Number(e.target.value))} className="w-full mt-5" />
    ...
    <section className="max-w-7xl mx-auto px-6 py-16">
      <div className="rounded-[2.5rem] bg-white/10 border border-white/10 p-8 md:p-12 grid lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
          ...
          <p className="text-white/70 mt-5 text-lg leading-relaxed">Connexion Supabase. Exercices renouvelés à chaque session.</p>
        </div>
...
function DemoModal({ setShowDemo }) {
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/80 flex items-center justify-center p-6">
      <motion.div initial={{ opacity: 0, y: 20, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} className="w-full max-w-3xl rounded-[2rem] bg-white text-slate-950 p-8 shadow-2xl">
        ...
        <div className="rounded-3xl bg-slate-950 text-white p-6">
          ...
          <p className="text-white/70 mt-3">Après connexion, l’élève ouvre une vraie page de cours puis travaille exercice par exercice.</p>
        </div>
      </motion.div>
    </div>
  );
...
function LoginModal({ setShowLogin, userEmail, setUserEmail, password, setPassword, handleLogin, handleSignup }) {
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/80 flex items-center justify-center p-6">
      <motion.div initial={{ opacity: 0, y: 20, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} className="w-full max-w-md rounded-[2rem] bg-white text-slate-950 p-8 shadow-2xl">
        <div className="flex items-center justify-between mb-7">
          <div>
        ...
        <div className="mt-2 mb-6 rounded-2xl bg-slate-100 px-4 py-3 flex items-center gap-3">
          🔒
          <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="bg-transparent outline-none w-full" />
        </div>

        <Button onClick={handleLogin} className="w-full rounded-2xl h-12 bg-slate-950 hover:bg-slate-800 text-white">
          Accéder au tableau de bord
        </Button>
        <Button onClick={handleSignup} className="w-full rounded-2xl h-12 mt-3 bg-indigo-500 hover:bg-indigo-400 text-white">
          Créer un compte
        </Button>
